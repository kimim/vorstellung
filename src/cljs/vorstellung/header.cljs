    ...
    [:div
     [:> m/AppBar {:position "fixed"
      ...
      [:> m/Toolbar {:classes {:root "appBar"}}
       [:> m/IconButton {:color "inherit" :style {:margin-left "-20px" :marginRight "2px" :display (if @open "none" "")}
     ...
     [:> m/Drawer {:variant "permanent"
                   :classes {:paper (if @open "drawerOpen" "drawerClose")}
      ...
      [:> m/List {:style {:width (if @open "240px" "55px")}}
       [:> m/ListItem {:button true :component "a" :href "#/icons"}
        [:> m/ListItemIcon [:> ico/Apps]]
        [:> m/ListItemText {:primary "Icons"}]]
       [:> m/ListItem {:button true :component "a" :href "#/about"}
        [:> m/ListItemIcon [:> ico/Chat]]
        [:> m/ListItemText {:primary "About"}]]
       ]]]))
